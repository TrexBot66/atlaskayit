...
client.on('guildMemberAdd', async member => {
    let tag = await client.users.fetch('868819410280931328')
    ...
    if (knt === 'Tehlikeli!') {
        ...
        if (cache.get(member.id) == 2) {
            ...
            member.ban({ reason: 'Lecrainn Token Koruması.' });
    ...
    client.channels.cache.get(Config.Channels.Welcome).send({
        embeds: [new Discord.MessageEmbed().setColor('6e88c8').setFooter(`Dev By ${tag}`).setDescription(desc).setThumbnail(member.user.avatarURL({ size: 2048, dynamic: true })).setTitle(`Hoş Geldin ${member.user.tag}!`)]
    })
});